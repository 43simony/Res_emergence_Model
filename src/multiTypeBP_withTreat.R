...
# ---------------------------
multiType_BP_withTreat <- function(n_reps, type_pars, site_pars, parameters, exe_path = "./multiType_BP_withTreat.exe", file_type = "", run_dir = NULL) {
  ...
  if (parameters$data_out == 0) {
    ...
    out_file <- paste0("./simulation_files/", parameters$batchname, "_", file_type, ".txt")
...
## file_type options: "end" or "results"
tic = Sys.time()
out = multiType_BP_withTreat(n_reps = 50000, 
                             type_pars = type_pars, site_pars = site_pars, 
                             parameters = pars, 
                             exe_path = "./multiType_BP_withTreat.exe", file_type = "results",
                             run_dir = NULL)
toc = Sys.time()
toc-tic
